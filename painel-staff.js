...
function selecionadas(){
  ...
  if (cat === 'orgs') return orgs.filter(o => categoriaDaOrg(o) === 'orgs' && o.tipo === livreOrgao.value);
  return orgs.filter(o => categoriaDaOrg(o) === cat);
}
function orgSelecionada(){ return orgs.find(o => o.id === livreDivisao.value); }
...
function atualizarDivisoesOrgao(){
  ...
  livreDivisao.innerHTML = lista.map(o => `<option value="${o.id}">${o.tipo} - ${o.nome} (${o.status})</option>`).join('');
  const sel = orgSelecionada();
  choicePreview.textContent = sel ? `Selecionado: ${sel.tipo} - ${sel.nome} (${sel.status})` : 'Nenhuma opção cadastrada nessa categoria.';
}
function renderLivres(){
  const box = document.getElementById('listasLivres');
  box.innerHTML = '';
  const grupos = {};
  orgs.forEach(o => { const k = categoriaDaOrg(o) === 'orgs' ? o.tipo : (categoriaDaOrg(o) === 'armas' ? 'FAC ARMAS' : 'FAC MUNIÇÕES'); (grupos[k] ||= []).push(o); });
  Object.entries(grupos).forEach(([nome, lista]) => {
    const card = document.createElement('div');
    card.className = 'request-card page-fade';
    card.innerHTML = `<strong>${nome}</strong><p>${lista.map(o => `${o.nome} (${o.status})`).join(' • ')}</p>`;
...
function renderFormularios(){
  ...
  box.innerHTML = '';
  if (!formularios.length){ box.innerHTML = '<p class="empty">Nenhum formulário enviado ainda.</p>'; return; }
  formularios.forEach((f) => {
    const card = document.createElement('div');
    card.className = 'request-card page-fade';
    card.innerHTML = `<strong>${f.orgs ? `${f.orgs.tipo} - ${f.orgs.nome}` : 'ORG'} — ${f.nome_grupo}</strong>
      ...
      <div class="mini-actions"><button onclick="mudarStatusForm('${f.id}', 'aprovado')">Aprovar</button><button onclick="mudarStatusForm('${f.id}', 'reprovado')">Recusar</button><button onclick="mudarStatusForm('${f.id}', 'pendente')">Pendente</button></div>`;
    box.appendChild(card);
  });
}
function renderControle(){
  ...
  box.innerHTML = `<div class="org-card">${orgs.length}<br><small>fac/org cadastradas</small></div><div class="org-card">${livres}<br><small>livres</small></div><div class="org-card">${privadas}<br><small>privadas</small></div><div class="org-card">${formularios.length}<br><small>formulários</small></div><div class="org-card">${aprovadas.length}<br><small>aprovadas</small></div><div class="org-card">${pendentes.length}<br><small>pendentes</small></div>`;
}
...
livreDivisao.addEventListener('change', () => { const sel = orgSelecionada(); choicePreview.textContent = sel ? `Selecionado: ${sel.tipo} - ${sel.nome} (${sel.status})` : 'Selecione uma opção.'; });
...
criarTipo.addEventListener('change', () => { criarDivisoesBox.style.display = criarTipo.value.startsWith('org_') ? 'grid' : 'none'; });
document.getElementById('criarForm').addEventListener('submit', async e => {
  ...
  try {
    if (tipo.startsWith('org_')) {
      ...
      for (const divisao of divs) await api('/api/orgs', { method: 'POST', body: JSON.stringify({ categoria, tipo: tipoBanco, nome: divisao, status: 'livre' }) });
    } else {
      await api('/api/orgs', { method: 'POST', body: JSON.stringify({ categoria, tipo: tipoBanco, nome: nome.toUpperCase(), status: 'livre' }) });
    }
    e.target.reset(); criarDivisoesBox.style.display = 'none'; msg.textContent = 'Criado no sistema com sucesso.'; msg.style.color = '#35ff7c';
    await carregarPainel();
  } catch (err) { msg.textContent = err.message; msg.style.color = '#ff6969'; }
});
